...
begin
  ...
  return jsonb_build_object(
    'current_cycle_paid', case when billing_mode = 'one_time' then unexpired_credit_package else paid_cycle.id is not null end,
...
credit_rollup as (
  select
    ...
    coalesce(sum(g.expired_credits), 0)::integer as expired_unused_credits
...
signals as (
  select
    ...
    greatest(
      coalesce(cr.active_credits, 0) + r.extra_capacity - r.lost_credits,
      0
    ...
    case
      when greatest(coalesce(cr.active_credits, 0) + r.extra_capacity - r.lost_credits, 0) >= 3 then 'green'
      when greatest(coalesce(cr.active_credits, 0) + r.extra_capacity - r.lost_credits, 0) >= 1 then 'yellow'
      else 'red'
