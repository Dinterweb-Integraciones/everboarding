...
declare
  ...
  billing_mode text;
  resolved_cycle_start date;
  resolved_cycle_end date;
begin
  ...
  resolved_cycle_start := cycle_window.cycle_start_date;
  resolved_cycle_end := cycle_window.cycle_end_date;
  ...
  if paid_cycle.id is null and billing_mode = 'one_time' then
    select *
    into paid_cycle
    from public.client_billing_cycles
    where client_id = p_client_id
    ...
    order by cycle_end_date desc, paid_at desc nulls last, created_at desc
    limit 1;

    if paid_cycle.id is not null then
      resolved_cycle_start := paid_cycle.cycle_start_date;
      resolved_cycle_end := paid_cycle.cycle_end_date;
    end if;
  end if;
  ...
  return jsonb_build_object(
    ...
    'current_cycle_start', resolved_cycle_start,
    'current_cycle_end', resolved_cycle_end,
