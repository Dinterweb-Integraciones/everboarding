...
client_rollup as (
  ...
  left join initiative_credits i
    on i.client_id = c.id
  group by c.id, c.name, c.slug, c.csm_user_id, c.created_at, csm.full_name, csm.email, config.start_date, config.custom_plan_billing_mode, config.lost_credits, config.extra_capacity, config.north_star_status
),
credit_rollup as (
  select
    g.client_id,
