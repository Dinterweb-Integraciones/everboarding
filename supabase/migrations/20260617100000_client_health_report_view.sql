...
client_rollup as (
  ...
  group by c.id, c.name, c.slug, c.created_at, config.start_date, config.custom_plan_billing_mode, config.lost_credits, config.extra_capacity, north.north_stars_completed
),
credit_rollup as (
  select
    g.client_id,
