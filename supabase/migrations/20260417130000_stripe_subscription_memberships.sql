-- Adds Stripe subscription tracking for negotiated multi-month memberships.

alter table public.client_billing_cycles
add column if not exists stripe_subscription_id text,
add column if not exists stripe_invoice_id text;

create index if not exists client_billing_cycles_stripe_subscription_idx
on public.client_billing_cycles (stripe_subscription_id);

create index if not exists client_billing_cycles_stripe_invoice_idx
on public.client_billing_cycles (stripe_invoice_id);

drop function if exists public.record_stripe_checkout_payment(uuid, text, text, integer, text);
